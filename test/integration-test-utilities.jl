...
now_localzone() = TimeZones.now(TimeZones.localzone())
            ...
            for _ in 1:(ceil(Int, length(branches_to_delete) / m) + 1)
                ...
                if !isempty(current_branches)
                    try
                        ...
                        @info "Encountered an error while trying to delete multiple branches" exception = (
                            ex, catch_backtrace()
                        ) current_branches
                    end
                end
...
function generate_master_branch(
    path_to_content::AbstractString, parent_branch::AbstractString="master"; repo_url
)
    name = "master"
...
function username(auth::GitHub.Authorization)
    sleep(5)
    user_information = GitHub.gh_get_json(GitHub.DEFAULT_API, "/user"; auth=auth)
    sleep(5)
...
function utc_to_string(zdt::TimeZones.ZonedDateTime)
    ...
    return @sprintf "%04d-%02d-%02d-%02d-%02d-%02d-%03d" year month day hour minute second millisecond
end

function with_master_branch(
    f::Function, path_to_content::AbstractString, parent_branch::AbstractString; repo_url
)
    b = generate_master_branch(path_to_content, parent_branch; repo_url=repo_url)
    result = f(b)
