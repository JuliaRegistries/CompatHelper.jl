@inline old_compat_to_new_compat(old_compat::Nothing, new_compat::String, keep_or_drop::Symbol) = "$(strip(new_compat))"

@inline function old_compat_to_new_compat(old_compat::String, new_compat::String, keep_or_drop::Symbol)
...
function make_pr_for_new_version(
    ...
    elseif !bump_compat_containing_equality_specifier &&
        ...
        if isnothing(current_compat_entry)
            brand_new_compat = old_compat_to_new_compat(nothing, compat_entry_for_latest_version, :brandnewentry)
            ...
            if drop_existing_compat
                drop_compat = old_compat_to_new_compat(current_compat_entry_verbatim, compat_entry_for_latest_version, :drop)
                make_pr_for_new_version(
                    api,
                    clone_hostname,
                    compat_entry_for_latest_version,
            ...
            if keep_existing_compat
                keep_compat = old_compat_to_new_compat(current_compat_entry_verbatim, compat_entry_for_latest_version, :keep)
                make_pr_for_new_version(
                    api,
                    clone_hostname,
                    compat_entry_for_latest_version,
...
function make_pr_for_new_version(
    ...
    pr_title_parenthetical = generate_pr_title_parenthetical(keep_or_drop, parenthetical_in_pr_title)
    ...
    if new_pr_title in pr_titles
        ...
        open(project_file, "w") do io
            TOML.print(io, project; sorted=true, by=key -> (Pkg.Types.project_key_order(key), key))
        end
